...
## 최종 해결: 항상 쓰기 가능한 위치에서 실행 (2026-03-30)

### 증상

`Data/` 디렉토리 제외 후에도 운영 빌드에서 관리자 권한 없이는 스캐너가 동작하지 않음. NAPS2가 `NAPS2_DATA` 환경변수와 무관하게 설치 디렉토리(`C:\Program Files\`)에 Data/, recovery, temp 등을 생성하려 시도.

### 원인

`NAPS2_DATA` 환경변수는 NAPS2의 **일부** 쓰기만 리다이렉트한다. NAPS2는 실행 파일 주변(exe 상위 디렉토리)에도 런타임 파일을 생성하려 하며, 이는 `NAPS2_DATA`로 제어되지 않는다.

기존 `ensureNonPortable()` 메서드가 `NAPS2.Portable.exe` 마커가 있을 때만 쓰기 가능한 위치로 복사본을 만들었는데, 마커가 빌드 타임에 제거되므로 이 로직이 작동하지 않았다.

### 해결

`ensureNonPortable()` → `ensureWritableNaps2()`로 변경하여, **포터블 마커 유무와 관계없이 항상** `{userData}/naps2-app/`에 복사본을 생성:

- exe와 설정 파일만 복사 (~170KB)
- `lib/` 디렉토리는 원본으로의 디렉토리 정션(junction) 생성 (151MB 복사 회피)
- `.version` 파일 비교로 앱 업데이트 시에만 복사본 갱신
- 복사 실패 시 원본 경로로 폴백
...
### 7. 서드파티 도구는 항상 쓰기 가능한 위치에서 실행하라

환경변수나 설정 파일로 쓰기 경로를 리다이렉트하는 것은 완전한 해결책이 아니다. 서드파티 도구가 실행 파일 주변에 예측 불가능한 파일을 생성할 수 있다. **실행 파일 자체를 쓰기 가능한 위치로 복사**하는 것이 가장 확실한 방법이다.
